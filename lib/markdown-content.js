...
Most endpoints require the \`campusx_token\` JWT cookie (jose HS256, HTTP-only). Log in with \`POST /api/auth/login\` (identifier +
...
const AUTH_MD = `# CampusZen Agent Auth

How AI agents obtain and use credentials to call the CampusZen API.

CampusZen is the social network for Indian college students. This guide
explains how an AI agent authenticates against the CampusZen REST API and
MCP servers.
...
1. **Session cookie** (primary): \`campusx_token\` JWT (jose HS256, HTTP-only,
   global logout via tokenVersion, single-device via TTL TokenBlacklist), obtained via \`POST /api/auth/login\`.
...
Cookie: campusx_token=<value>
\`\`\`

## Sandbox / test mode

Agents can exercise the API without touching production data by sending the
\`X-CampusZen-Test-Mode: true\` header (or \`?sandbox=1\`) on any request.
Sandbox responses are tagged with an \`X-Sandbox: true\` header and operate on
isolated, non-destructive sample data, so agents can dry-run calls and script
interactions safely. CampusZen also offers a free tier with self-serve API
access — no sales contact required.
...
const API_CATALOG_MARKDOWN = `# CampusZen API Catalog (RFC 9727)

A linkset of CampusZen's machine-readable API descriptions.

- OpenAPI: https://campuszen.tech/openapi.json
- MCP server: https://campuszen.tech/.well-known/mcp
- MCP docs server: https://campuszen.tech/.well-known/mcp-docs
- Agent card (A2A): https://campuszen.tech/.well-known/agent-card.json
- Agent skills: https://campuszen.tech/.well-known/agent-skills/index.json
- llms.txt: https://campuszen.tech/llms.txt
- Auth guide: https://campuszen.tech/auth.md
`;

const API_INDEX_MARKDOWN = `# CampusZen API

CampusZen exposes a public REST API plus an MCP server.

## Public endpoints (no auth)

- \`GET /api/communities\` — list or fetch a community
- \`GET /api/public/stats\` — platform counts
- \`GET /api/health\` — liveness probe
- \`GET /api/leaderboard\` — top contributors
- \`GET /api/events\` — upcoming events
...
Full schema: https://campuszen.tech/openapi.json
Agent auth: https://campuszen.tech/auth.md
`;

function apiEndpointMarkdown(p) {
    return `# CampusZen API endpoint: ${p}

This is the markdown twin for the \`${p}\` endpoint. The full request and
response schema is documented in the OpenAPI spec:
https://campuszen.tech/openapi.json

Agent auth guide: https://campuszen.tech/auth.md
`;
}

// Returns a markdown representation for a path with a trailing `.md` stripped.
// Used by middleware so agents can fetch `page.md` twins. Returns null when
// there is no known markdown twin (so the request falls through).
export function getMdTwinContent(basePath) {
    if (!basePath) return null;
    if (basePath === "/.well-known/api-catalog") return API_CATALOG_MARKDOWN;
    if (basePath === "/api") return API_INDEX_MARKDOWN;
    if (basePath.startsWith("/api/")) return apiEndpointMarkdown(basePath);
    if (PUBLIC_MARKDOWN_PATHS.has(basePath) || basePath.startsWith("/community/")) {
        return getMarkdownContent(basePath);
    }
    return null;
}
