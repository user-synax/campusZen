...
// ━━━ FEEDBACK ━━━ //
export async function notifyAdminFeedback({ message, user = null, path = null }) {
  if (!process.env.GMAIL_USER || !process.env.GMAIL_APP_PASS || !process.env.ADMIN_NOTIFY_EMAIL) {
    console.warn('[FeedbackNotify] Skipping — email env vars not set')
    return
  }
  try {
    const safeMessage = sanitizeForHTML(message)
    const safeName = user ? sanitizeForHTML(user.name) : 'Anonymous'
    const safeUsername = user ? sanitizeForHTML(user.username) : '—'
    const safeEmail = user ? sanitizeForHTML(user.email) : '—'
    const safePath = path ? sanitizeForHTML(path) : '—'

    const mailer = getTransporter()
    await mailer.sendMail({
      from: `"CampusZen Feedback 🎓" <${process.env.GMAIL_USER}>`,
      to: process.env.ADMIN_NOTIFY_EMAIL,
      subject: `💬 New feedback from ${safeName}`,
      text: `New feedback from ${safeName} (@${safeUsername} — ${safeEmail}) on ${safePath}:\n\n${message}`,
      html: buildFeedbackHTML({ safeMessage, safeName, safeUsername, safeEmail, safePath }),
    })
    console.log('[FeedbackNotify] ✅ Sent feedback')
  } catch (error) {
    console.error('[FeedbackNotify] ❌ Failed:', error.message)
    throw error
  }
}

function buildFeedbackHTML({ safeMessage, safeName, safeUsername, safeEmail, safePath }) {
  return `
    <!DOCTYPE html>
    <html lang="en">
      ...
      <div style="max-width:520px;margin:0 auto;padding:32px 16px;">
        <div style="background:#0f0f0f;border:1px solid #1f1f1f;border-radius:16px;overflow:hidden;">
          <div style="background:linear-gradient(135deg,#111 0%,#0f0f0f 100%);border-bottom:1px solid #1a1a1a;padding:24px 28px;">
            ...
            <span style="float:right;background:#1a1000;color:#f59e0b;border:1px solid #f59e0b30;padding:3px 10px;border-radius:999px;font-size:11px;font-weight:700;">FEEDBACK</span>
          </div>
          <div style="padding:24px 28px;">
            <p style="margin:0 0 16px;color:#888;font-size:13px;">New feedback submitted</p>
            <div style="background:#111;border:1px solid #1c1c1c;border-radius:12px;padding:16px;color:#d1d5db;font-size:14px;line-height:1.6;white-space:pre-wrap;">${safeMessage}</div>
            <table style="width:100%;border-collapse:collapse;margin-top:20px;">
              <tr><td style="padding:10px 0;color:#555;font-size:11px;text-transform:uppercase;letter-spacing:0.8px;font-weight:700;">From</td><td style="padding:10px 0;color:#d1d5db;font-size:14px;text-align:right;">${safeName} <span style="color:#444;font-family:'Courier New',monospace;">@${safeUsername}</span></td></tr>
              <tr><td style="padding:10px 0;color:#555;font-size:11px;text-transform:uppercase;letter-spacing:0.8px;font-weight:700;">Email</td><td style="padding:10px 0;color:#d1d5db;font-size:14px;text-align:right;">${safeEmail}</td></tr>
              <tr><td style="padding:10px 0;color:#555;font-size:11px;text-transform:uppercase;letter-spacing:0.8px;font-weight:700;">Page</td><td style="padding:10px 0;color:#d1d5db;font-size:13px;text-align:right;font-family:'Courier New',monospace;">${safePath}</td></tr>
            </table>
          </div>
          <p style="text-align:center;color:#2a2a2a;font-size:11px;margin:20px 0 0;">Only you receive this · CampusZen Feedback</p>
        </div>
      </div>
    </body>
    </html>
