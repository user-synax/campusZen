...
export function getNotificationText(type, senderName, meta) {
  ...
  const messages = {
    ...
    connected:        `${name} connected with you`,
...
export function getNotificationURL(notification) {
  ...
  switch(type) {
    ...
    case 'connected':
      return sender?.username ? `/chats/dm/${meta?.conversationId || ''}` : '/connect'
    default:
...
export async function createNotification({
  ...
  // RULE 1: Never notify yourself (unless it's a system notification)
  const isSystemType = ['system'].includes(type)
  ...
  try {
    ...
    if (dedupe) {
      ...
      // Populate sender for realtime payload
      ...
      sendPushToUser(recipient, {
        title: populated.sender?.name || 'CampusZen',
...
async function pushNotification(recipientId, notification) {
  try {
    ...
    // Realtime failure = silent — notification is in DB, user will see on next load
    console.error('[Notification] realtime push failed:', err.message)
  }
}
...
export async function deleteNotification({
  ...
  try {
    ...
    await emitToUser(
      ...
      'notification:remove',
      { dedupeKey }
    ).catch(err => console.error('[realtime] notification:remove failed:', err))
