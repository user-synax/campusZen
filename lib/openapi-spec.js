...
export const openapiSpec = {
    ...
    info: {
        ...
        description:
            ...
            "## Authentication\n" +
            "Most endpoints require the `campusx_token` HTTP-only JWT cookie signed with `jose` (HS256) and enforced via `tokenVersion` for global logout and a TTL `TokenBlacklist` for single-device logout. Errors are structured JSON: `{ \"success\": false, \"error\": { \"code\": string, \"message\": string }, \"timestamp\": string }`.\n\n" +
            "## Sandbox / test mode\n" +
    ...
    components: {
        securitySchemes: {
            sessionCookie: {
                ...
                name: "campusx_token",
                description:
                    "JWT session cookie (campusx_token) signed with jose HS256. Global logout via User.tokenVersion, single-device logout via TTL TokenBlacklist.",
            },
            oauth2: {
                type: "oauth2",
                description:
                ...
                flows: {
                    clientCredentials: {
                        ...
                        scopes: {
                            "read:public": "Read public platform data (stats, events).",
