...
export async function purchaseItem(userId, identifier) {
    try {
        ...
        // Charge VP (atomic, conditional). Amount from catalog, never client.
        ...
        if (!spent.spent) {
            return { purchased: false, reason: spent.reason };
        }

        // Grant item (self-contained snapshot for rendering).
        ...
        // Auto-equip on purchase (one item per category).
        await ShopItem.findByIdAndUpdate(item._id, { $inc: { soldCount: 1 } });
