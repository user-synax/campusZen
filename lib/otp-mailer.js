 ...
 * Resilient across Node, Edge, and serverless runtimes.
 */
export function generateOTP() {
  try {
    // Prefer Web Crypto (available in Node 19+, Edge, browsers)
    const gCrypto =
      globalThis.crypto ||
  ...
  try {
    // Node.js fallback — synchronous randomInt if available
    // Dynamic require to avoid bundling issues on Edge
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const nodeCrypto = typeof require !== 'undefined' ? require('crypto') : null
    if (nodeCrypto) {
      if (typeof nodeCrypto.randomInt === 'function') {
        return String(nodeCrypto.randomInt(0, 1000000)).padStart(6, '0')
      }
  ...
  // Last resort — not cryptographically strong but prevents 500
