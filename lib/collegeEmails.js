...
/**
 * Extract college domain from an email (lowercased).
 * Returns domain string or null.
 */
export function getCollegeDomain(email) {
    return extractDomain(email);
}

/**
 * Map college name back to its canonical domain (reverse lookup).
 * Returns domain string or empty string if unknown.
 */
export function getDomainForCollege(collegeName) {
    if (!collegeName) return "";
    const target = collegeName.trim().toLowerCase();
    for (const [domain, name] of Object.entries(COLLEGE_DOMAINS)) {
        if (name.toLowerCase() === target) return domain;
    }
    return "";
}
