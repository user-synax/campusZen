...
// Deprecated gamification / shop fields — kept in DB for migration but must not leak via API.
// These were removed in v2.0 cleanup (xp, vp, shop) but still exist on old documents.
const DEPRECATED_USER_FIELDS = [
  'xp', 'level', 'totalXP', 'weeklyXP', 'vp', 'lastLoginRewardAt',
]
...
export function sanitizeUser(userDoc) { 
  ...
  DEPRECATED_USER_FIELDS.forEach(field => delete obj[field])
  // keep ownedShopItems/equippedShopItems for cosmetic frames until fully pruned, but strip xp/vp
  return obj 
}
...
export function stripDeprecatedUserFields(user) {
  if (!user) return user;
  const clone = { ...user };
  DEPRECATED_USER_FIELDS.forEach(f => delete clone[f]);
  return clone;
} 
